...
#pragma warning(disable:4838)		// conversion from 'int' to 'const unsigned char' requires a narrowing conversion
#include <sstream>
...
extern bool jo_write_jpg(std::stringstream & out, const void *data, int width, int height, int comp, int quality );
...
static void jo_writeBits(std::stringstream & out, int &bitBuf, int &bitCnt, const unsigned short *bs) 
{
	bitCnt += bs[1];
...
static int jo_processDU(std::stringstream & out, int &bitBuf, int &bitCnt, float *CDU, float *fdtbl, int DC, const unsigned short HTDC[256][2], const unsigned short HTAC[256][2]) {
...
bool jo_write_jpg(std::stringstream & out, const void *data, int width, int height, int comp, int quality) {
	...
	out << (unsigned char)0x10; // HTYACinfo
	...
	out << (unsigned char)0x11; // HTUACinfo
	...
	// EOI
	out << (unsigned char)0xFF;
	out << (unsigned char)0xD9;
